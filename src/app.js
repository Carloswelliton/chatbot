...
async function iniciarBot() {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    for (const msg of messages) {
      ...
      switch (s.step) {
        ...
        case 1:
          const clientMsg = text
            ...
            .replace(/[\u0300-\u036f]/g, '');
          if (
            ['1', 'um', 'caes', 'cachorro', 'servicos para caes'].some((palavra) =>
              clientMsg.includes(palavra)
            )
          ) {
            ...
            [
              ...
              'serviços para gatos',
              'servicos para gatos',
            ].some((palavra) => clientMsg.includes(palavra))
          ) {
            ...
            ['3', 'tres', 'produtos', 'produto', 'loja'].some((palavra) =>
              clientMsg.includes(palavra)
            )
          ) {
            s.step = 2003;
            ...
            ['4', 'quatro', 'agendar', 'agendamento', '', 'consulta', 'agendar consulta'].some(
              (palavra) => clientMsg.includes(palavra)
            )
          ) {
            s.step = 2004;
            await safeSendMessage(sock, sender, {
            ...
            await safeSendMessage(sock, sender, {
              text: `❌ Opção inválida, por favor informe uma opção válida`,
            });
          }
          break;
        case 2001:
          const serviceSelectedDog = text
            .toLowerCase()
            .normalize('NFD')
            .replace(/[\u0300-\u036f]/g, '');
          if (
            ['1', 'um', 'banho', 'banho completo'].some((palavra) =>
              serviceSelectedDog.includes(palavra)
            )
          ) {
            s.selectedService = 'Banho completo para Cães';
            await askServiceDetails(sock, sender, s);
          } else if (
            ['2', 'dois', 'tosa', 'tosa higienica'].some((palavra) =>
              serviceSelectedDog.includes(palavra)
            )
          ) {
            ...
            ['3', 'tres', 'tosa estetica'].some((palavra) => serviceSelectedDog.includes(palavra))
          ) {
            s.selectedService = 'Tosa estética para Cães';
            await askServiceDetails(sock, sender, s);
          } else if (
            ['4', 'quatro', 'banho', 'Banho terapêutico'].some((palavra) =>
              serviceSelectedDog.includes(palavra)
            )
          ) {
            ...
            ['5', 'cinco', 'sair', 'voltar'].some((palavra) => serviceSelectedDog.includes(palavra))
          ) {
            s.step = 1;
      ...
      async function showMainMenu(sock, sender, session) {
        session.step = '';
        await safeSendMessage(sock, sender, {
          text:
            `😊 Muito prazer, ${s.name}! Por favor, digite o número da categoria de atendimento que deseja:\n\n` +
